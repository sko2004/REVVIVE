...
def insert_run(run_id: UUID, user_message: str) -> None:
    with connection() as conn:
        conn.execute(
            "INSERT INTO agent_runs (id, user_message, status) VALUES (%s, %s, %s)",
            (str(run_id), user_message, "running"),
        )
        conn.commit()
...
def list_recent_runs(limit: int = 20) -> list[dict[str, Any]]:
    with connection() as conn:
        cur = conn.execute(
            """
            SELECT id, created_at, user_message, assistant_final, status, error_message
            FROM agent_runs
